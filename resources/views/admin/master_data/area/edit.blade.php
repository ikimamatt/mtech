...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Ubah Area</h6>
                    <form class="forms-sample" action="{{ route('area.update', ['area' => $area]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select id="select-region" name="kd_region" class="form-select @error('kd_region') is-invalid @enderror" data-width="100%"  >                        
                                ...
                                @foreach ($regions as $region)
                                    <option value="{{$region->kd_region}}" {{old('kd_region',$area->kd_region) == $region->kd_region?'selected':''}}>{{$region->nama_region}}</option>
                                @endforeach
                            </select>
                            @error('kd_region')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('kd_area') is-invalid @enderror"
                                name="kd_area" id="inputKodeArea" placeholder="Kode Area"
                                value="{{ old('kd_area', $area->kd_area) }}">
                            @error('kd_area')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama_area') is-invalid @enderror"
                                name="nama_area" id="inputNamaArea" placeholder="Nama Area"
