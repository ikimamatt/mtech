...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Ubah Kontrak Jaringan Icon+</h6>
                    <form class="forms-sample"
                        action="{{ route('network-contract-icon.update', ['network_contract_icon' => $network_contract_icon]) }}"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('activation_date') is-invalid @enderror"
                                name="activation_date" id="activation_date"
                                value="{{ old('activation_date', $network_contract_icon->activation_date) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('service_id') is-invalid @enderror"
                                name="service_id" id="service_id" placeholder="ID Service"
                                value="{{ old('service_id', $network_contract_icon->service_id) }}">
                            @error('service_id')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="service" name="service" class="form-select @error('service') is-invalid @enderror"
                                ...
                                <option value="IPVPN"
                                    {{ old('service', $network_contract_icon->service) == 'IPVPN' ? 'selected' : '' }}>
                                    IPVPN</option>
                                <option value="Metronet"
                                    {{ old('service', $network_contract_icon->service) == 'Metronet' ? 'selected' : '' }}>
                                    Metronet</option>
                                <option value="VSAT IP"
                                    {{ old('service', $network_contract_icon->service) == 'VSAT IP' ? 'selected' : '' }}>
                                    VSAT IP</option>
                                <option value="Clear Channel"
                                    {{ old('service', $network_contract_icon->service) == 'Clear Channel' ? 'selected' : '' }}>
                                    Clear Channel</option>
                                <option value="Internet"
                                    {{ old('service', $network_contract_icon->service) == 'Internet' ? 'selected' : '' }}>
                                    Internet</option>
                            </select>
                            @error('service')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="asman" name="asman"
                                ...
                                <option value="Asman Sumut 1" {{ old('asman', $network_contract_icon->asman) == 'Asman Sumut 1' ? 'selected' : '' }}>
                                    Asman Sumut 1</option>
                                <option value="Asman Sumut 2" {{ old('asman', $network_contract_icon->asman) == 'Asman Sumut 2' ? 'selected' : '' }}>
                                    Asman Sumut 2
                                </option>
                            </select>
                            @error('asman')
                            ...
                            <select id="unit_id" name="unit_id" class="form-select @error('unit_id') is-invalid @enderror"
                                ...
                                @foreach ($units as $unit)
                                    <option value="{{ $unit->id }}"
                                        {{ old('unit_id', $network_contract_icon->unit_id) == $unit->id ? 'selected' : '' }}>
                                        {{ $unit->nama_unit }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('explanation') is-invalid @enderror" name="explanation" id="explanation"
                                placeholder="Keterangan">{{ old('explanation', $network_contract_icon->explanation) }}</textarea>
                            @error('explanation')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('activation_number') is-invalid @enderror"
                                ...
                                value="{{ old('activation_number', $network_contract_icon->activation_number) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="scada" name="scada" class="form-select @error('scada') is-invalid @enderror"
                                ...
                                <option value="Scada"
                                    {{ old('scada', $network_contract_icon->scada) == 'Scada' ? 'selected' : '' }}>
                                    Scada</option>
                                <option value="Non Scada"
                                    {{ old('scada', $network_contract_icon->scada) == 'Non Scada' ? 'selected' : '' }}>Non
                                    Scada
                                </option>
                            </select>
                            @error('scada')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('capacity') is-invalid @enderror"
                                ...
                                value="{{ old('capacity', $network_contract_icon->capacity) }}">
                            @error('capacity')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('price') is-invalid @enderror"
                                ...
                                value="{{ old('price', $network_contract_icon->price) }}">
                            @error('price')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="status" name="status"
                                ...
                                <option value="Activate"
                                    {{ old('status', $network_contract_icon->status) == 'Activate' ? 'selected' : '' }}>
                                    Activate</option>
                                <option value="Deactivated"
                                    {{ old('status', $network_contract_icon->status) == 'Deactivated' ? 'selected' : '' }}>
                                    Deactivated
                                </option>
                            </select>
                            @error('status')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="month" name="month"
                                ...
                                <option value="Januari"
                                    {{ old('month', $network_contract_icon->month) == 'Januari' ? 'selected' : '' }}>
                                    Januari</option>
                                <option value="Februari"
                                    {{ old('month', $network_contract_icon->month) == 'Februari' ? 'selected' : '' }}>
                                    Februari</option>
                                <option value="Maret"
                                    {{ old('month', $network_contract_icon->month) == 'Maret' ? 'selected' : '' }}>
                                    Maret</option>
                                <option value="April"
                                    {{ old('month', $network_contract_icon->month) == 'April' ? 'selected' : '' }}>
                                    April</option>
                                <option value="Mei"
                                    {{ old('month', $network_contract_icon->month) == 'Mei' ? 'selected' : '' }}>
                                    Mei</option>
                                <option value="Juni"
                                    {{ old('month', $network_contract_icon->month) == 'Juni' ? 'selected' : '' }}>
                                    Juni</option>
                                <option value="Juli"
                                    {{ old('month', $network_contract_icon->month) == 'Juli' ? 'selected' : '' }}>
                                    Juli</option>
                                <option value="Agustus"
                                    {{ old('month', $network_contract_icon->month) == 'Agustus' ? 'selected' : '' }}>
                                    Agustus</option>
                                <option value="September"
                                    {{ old('month', $network_contract_icon->month) == 'September' ? 'selected' : '' }}>
                                    September</option>
                                <option value="Oktober"
                                    {{ old('month', $network_contract_icon->month) == 'Oktober' ? 'selected' : '' }}>
                                    Oktober</option>
                                <option value="November"
                                    {{ old('month', $network_contract_icon->month) == 'November' ? 'selected' : '' }}>
                                    November</option>
                                <option value="Desember"
                                    {{ old('month', $network_contract_icon->month) == 'Desember' ? 'selected' : '' }}>
                                    Desember</option>
                            </select>
                            @error('month')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="year" name="year"
                                ...
                                @for ($i = date('Y'); $i <= 2030; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('year', $network_contract_icon->year) == $i ? 'selected' : '' }}>
                                        {{ $i }}</option>
                                @endfor
                            </select>
                            @error('year')
