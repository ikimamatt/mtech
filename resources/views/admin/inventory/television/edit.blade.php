...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('area.index') }}">Inventory (television)</a></li>
    ...
    <div class="row">
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">Tambah television</h6>
                    <form class="forms-sample" action="{{ route('television.update', ['television' => $television]) }}"
                        method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="select-region" class="form-label">Merk television</label>
                            <select id="select-region" name="brand_id"
                                class="form-select @error('brand_id') is-invalid @enderror" data-width="100%">
                                <option value="">-- Pilih Merek --</option>
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}"
                                        {{ old('id', $brand->id) == $television->brand_id ? 'selected' : '' }}>
                                        {{ $brand->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('serial_number') is-invalid @enderror"
                                ...
                                value="{{ old('serial_number', $television->serial_number) }}">
                            @error('serial_number')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('model') is-invalid @enderror" name="model"
                                id="inputModel" placeholder="Serial Number" value="{{ old('model', $television->model) }}">
                            @error('model')
                        ...
                        <div class="mb-3">
                            ...
                            <select id="select-region" name="kd_region"
                                ...
                                @foreach ($regions as $region)
                                    <option value="{{ $region->kd_region }}" @selected($region->kd_region == $television->kd_region)>
                                        {{ $region->nama_region }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="select-region-ownership" name="status_asset"
                                ...
                                <option value="Aset PLN" {{ $television->status_asset === 'Aset PLN' ? 'selected' : '' }}>
                                    Aset PLN</option>
                                <option value="Sewa" {{ $television->status_asset === 'Sewa' ? 'selected' : '' }}>Sewa
                                </option>
                            </select>
                            @error('status_asset')
                                <label for="select-region" class="error mt-1 tx-13 text-danger">{{ $message }}</label>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea type="text" class="form-control @error('keterangan') is-invalid @enderror" name="keterangan"
                                id="InputKeterangan" placeholder="Spesifikasi television" rows="3">{{ old('keterangan', $television->keterangan) }}</textarea>
                            @error('keterangan')
                        ...
                        <a href="{{ route('television.index') }}" class="btn btn-secondary" type="reset">Cancel</a>
