        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="forms-sample" action="{{ route('cctv.update', ['cctv' => $cctv]) }}"
                        method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="select-region" class="form-label">Merk CCTV</label>
                            <select id="select-region" name="brand_id"
                                class="form-select @error('brand_id') is-invalid @enderror" data-width="100%">
                                <option value="">-- Pilih Merek --</option>
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}"
                                        {{ old('id', $brand->id) == $cctv->brand_id ? 'selected' : '' }}>
                                        {{ $brand->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nama') is-invalid @enderror"
                                name="nama" id="input_nama" placeholder="Nama CCTV"
                                value="{{ old('nama', $cctv->nama) }}">
                            @error('nama')
                                <label for="input_nama" class="error mt-1 tx-13 text-danger">{{ $message }}</label>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('model') is-invalid @enderror" name="model"
                                id="inputModel" placeholder="Serial Number" value="{{ old('model', $cctv->model) }}">
                            @error('model')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('nomor_seri') is-invalid @enderror"
                                ...
                                value="{{ old('nomor_seri', $cctv->nomor_seri) }}">
                            @error('nomor_seri')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('alamat_ip') is-invalid @enderror"
                                name="alamat_ip" id="input_alamat_ip" placeholder="Alamat IP"
                                value="{{ old('alamat_ip', $cctv->alamat_ip) }}">
                            @error('alamat_ip')
                                <label for="input_alamat_ip" class="error mt-1 tx-13 text-danger">{{ $message }}</label>
                            @enderror
                        </div>
                       ...
                       <div class="mb-3">
                            ...
                            <select id="select-status_cctv" name="status_cctv"
                                ...
                                <option value="">-- Pilih Status_cctv --</option>
                                <option value="Aktif" {{ $cctv->status_cctv === 'Aktif' ? 'selected' : '' }}>
                                    Aktif</option>
                                <option value="Non Aktif" {{ $cctv->status_cctv === 'Non Aktif' ? 'selected' : '' }}>Non Aktif
                                </option>
                                <option value="Rusak" {{ $cctv->status_cctv === 'Rusak' ? 'selected' : '' }}>Rusak
                                </option>
                                <option value="Cadangan" {{ $cctv->status_cctv === 'Cadangan' ? 'selected' : '' }}>Cadangan
                                </option>
                            </select>
                            @error('status_cctv')
                                <label for="select-region" class="error mt-1 tx-13 text-danger">{{ $message }}</label>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="select-region" name="kd_region"
                                ...
                                @foreach ($regions as $region)
                                    <option value="{{ $region->kd_region }}" @selected($region->kd_region == $cctv->kd_region)>
                                        {{ $region->nama_region }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal_instalasi') is-invalid @enderror"
                                ...
                                value="{{ old('tanggal_instalasi',$cctv->tanggal_instalasi) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea type="text" class="form-control @error('keterangan') is-invalid @enderror" name="keterangan"
                                id="InputKeterangan" placeholder="Spesifikasi CCTV" rows="3">{{ old('keterangan', $cctv->keterangan) }}</textarea>
                            @error('keterangan')
